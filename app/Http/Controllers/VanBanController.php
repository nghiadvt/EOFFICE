...
class VanBanController extends Controller
{
    public function addVanBanDen()
    {
        ...
        return view(config('app.interface').'vanban.nhap_van_ban_den', compact('donvis', 'linhvucs', 'loaivanbans', 'publishs', 'sovanbandens', 'bookDetails', 'dataTreeview'));
    }
    ...
    public function edit_vanban($vanbanId) {
        ...
        return view(config('app.interface').'vanban.edit_van_ban_den', compact('donvis', 'linhvucs', 'loaivanbans', 'publishs', 'sovanbandens', 'bookDetails', 'vanban', 'userChuTris', 'userPhoiHops', 'userChuTriDaXemVanBanIds', 'userPhoiHopDaXemVanBanIds', 'donviPhoiHopDaXemVanBanIds', 'userTrongDonViPhoiHops'));
    }
    ...
    public function edit_vanban_di($vanbanId) {
        ...
        return view(config('app.interface').'vanban.edit_van_ban_di', compact('loaivanbans', 'donvischa', 'donviIds', 'linhvucs', 'bookDetails', 'vanban'));
        return view(config('app.interface').'vanban.edit_van_ban_den', compact('donvis', 'linhvucs', 'loaivanbans', 'publishs', 'sovanbandens', 'bookDetails', 'vanban', 'userChuTris', 'userPhoiHops', 'userChuTriDaXemVanBanIds', 'userPhoiHopDaXemVanBanIds', 'donviPhoiHopDaXemVanBanIds', 'userTrongDonViPhoiHops'));
    }
    ...
    public function save_vanban(Request $request) {
        ...
        if ($request->hasFile('files')) {
            ...
            foreach ($files as $file) {
				if (!in_array(strtolower($file->getClientOriginalExtension()), ['doc', 'docx', 'xls', 'xlsx', 'ppt', 'pptx', 'pdf', 'zip'])) {
            ...
            foreach ($files as $file) {
                ...
                foreach ($fileNameArr as $key => $value):
                    if (in_array(strtolower($value), ['doc', 'docx', 'xls', 'xlsx', 'ppt', 'pptx', 'pdf', 'zip'])) {
    ...
    public function getUser()
    {
        ...
        return view(config('app.interface').'vanban.ajax_user', compact('users'));
    }
    ...
    public function dsVanbanden(Request $request)
    {
        ...
        return view(config('app.interface').'vanban.danh_sach_van_ban_den', compact('status', 'linhvucs', 'loaivanbans', 'vanbans'));
    }
    ...
    public function van_ban_den_xu_ly()
    {
        ...
        return view(config('app.interface').'vanban.van_ban_den_xu_ly', compact('vanbanxulys'));
    }
    ...
    public function vanbandendagui()
    {
        ...
        return view(config('app.interface').'vanban.danh_sach_van_ban_den_da_gui', compact('lists', 'ds_vanbans', 'publishs', 'donvis', 'linhvucs', 'loaivanbans', 'book_detail'));
    }
    ...
    public function dsVanbandi(Request $request)
    {
        ...
        return view(config('app.interface').'vanban.danh_sach_van_ban_di', compact('vanbans', 'publishs', 'donvis', 'linhvucs', 'loaivanbans', 'donviGuiMails','donviSoan'));
    }
    ...
    public function VanBanDi()
    {
        ...
        return view(config('app.interface').'vanban.nhap_van_ban_di', compact('loaivanbans', 'donvischa', 'donviIds', 'linhvucs', 'bookDetails'));
    }
    ...
    public function settingUnit()
    {
        ...
        if ($role['role'] == 1) {
            ...
            $users = User::all();
            $books = Book::all();
            return view(config('app.interface').'vanban.cau_hinh_don_vi_nhan', compact('donvischa', 'linhvucs', 'loaivanbans', 'publishs', 'donvis', 'users', 'books'));
        } else {
            return view(config('app.interface').'vanban.test');
        }
    ...
    {
        return view(config('app.interface').'emails.vanbandi');
    }
    ...
    public function detailVanbanden($id) {
        ...
        if (!$vanbanden){
            return view(config('app.interface').'vanban.van_ban_khong_quyen_truy_cap');
        }
        ...
        return view(config('app.interface').'vanban.chi_tiet_van_ban_den', compact(
            'tab', 'vbxuly', 
    ...
    public function chi_tiet_van_ban_di($vanbanId) {
        ...
        return view(config('app.interface').'vanban.chi_tiet_van_ban_di', compact('vanban'));
    }
    ...
    public function chi_tiet_van_ban_den_da_gui($id)
    {
        ...
        return view(config('app.interface').'vanban.chi_tiet_van_ban_den_da_gui', compact('vanbanden', 'user'));
    ...
    public function dowloadFile($name)
    {
        ...
        return view(config('app.interface').'vanban.view-pdf',compact('files'));
//        return response()->download($files);
    ...
    public function dsVanbanluu()
    {
        ...
        return view(config('app.interface').'vanban.van_ban_luu', compact('lists', 'donvis', 'publishs'));
    }

    public function detailVanbanluu($id)
    {
        $vanbanden = Vanban::find($id);
        return view(config('app.interface').'vanban.chi_tiet_van_ban_den', compact('vanbanden'));
    }
    ...
    public function editVanbanluu($id)
    {
        ...
        return view(config('app.interface').'vanban.edit_van_ban_luu', compact('donvis', 'linhvucs', 'loaivanbans', 'publishs', 'book_detail', 'vanbans'));
    }
    ...
    public function edit_van_ban_da_gui($id)
    {
        ...
        return view(config('app.interface').'vanban.edit_van_ban_da_gui', compact('donvis', 'linhvucs', 'loaivanbans', 'publishs', 'book_detail', 'vanbans', 'userid', 'arrDv'));
    }
    ...
    public function gui_but_phe_van_ban_den($id)
    {
        ...
        return view(config('app.interface').'vanban.gui_but_phe_van_ban_den', compact('vanbanden', 'book_detail'));
    }
    ...
    public function view_log($IdVanBanUser)
    {
        $dataLog = Log::select('vanbans.cq_banhanh', 'vanbans.title', 'vanbans.file_dinhkem', 'vanbans.ngaygui', 'vanbans.hanxuly', 'logs.*', 'tbDonViNhan.name as tenDonViNhan', 'tbDviBanHanh.name as tenDViBanHanh')
            ...
            ->leftJoin('donvis as tbDviBanHanh', 'tbDviBanHanh.id', '=', 'vanbans.cq_banhanh')
            ->get();
        return view(config('app.interface').'vanban.view_log', compact('dataLog'));
    }
    ...
    public function chuyen_xu_ly_van_ban($id)
    {
        ...
        foreach ($arrayFalse as $val){
            array_push($dataTreeview, $val);
        }
        ...
        return view(config('app.interface').'vanban.chuyen_xu_ly_van_ban', compact('vanbanden', 'book_detail', 'IdVanBanUser','arrIdUser','dataTreeview', 'userChuTris', 'donviPhoihops', 'userPhoihops', 'bookDetails', 'vanbanxuly'));
    }
    ...
    public function luu_xu_ly_van_ban(Request $request)
    {
        ...
        $userIds_butphe = isset($request->users) ? $request->users : null;
        $userIds_chutri = isset($request->user_chutri_ids) ? $request->user_chutri_ids : null;
        if ($userIds_chutri){
            foreach ($userIds_chutri as $val){
                array_push($userIds_butphe, $val);
            }
        }
        $now = date('Y-m-d H:i:s');
        $inputs = Input::all();
        ...
        $butphe = Butphe::create([
            ...
            'receiver_ids' => ';'.trim(implode(';',$userIds_butphe)).';',
        ...
        if ($checkparentVanBan) {
            if ($userIds_chutri) {
                foreach ($userIds_chutri as $userId_chutri) {
                    if (!in_array($userId_chutri, $sentUserIds)) {
                        $user = User::find($userId_chutri);

                        $vbxlyId = VanbanXuLy::insertGetId([
                            'vanbanUser_id' => $idVanBan,
                            'parent_id' => $checkparentVanBan->id,
                            'id_nhan' => $userId_chutri,
                            'ngaychuyentiep' => date('Y-m-d H:i:s'),
...
//                            'type' => $checkparentVanBan->type
        ...
        $vanbans = Vanban::find($idVanBan);
        if ($userIds_chutri) {
            // get nội dung gửi mail
            $vanbans->title = 'Văn bản chuyển xử lý: '.$vanbans->title;
            $vanbans->donvisoan_id = $vanbans->cq_banhanh;
            $vanbans->loaivanban_id = Loaivanban::where('id', $vanbans->loaivanban_id)->first()->name;
            $vanbans->ngaydi = $vanbans->ngayky;
            $vanbans->link = route('chitiet_vanban', $idVanBan);
            $vanbans->path = url('/vanban/dowload_file');
            $vanbans->file = [$vanbans->file_dinhkem];
            $vanbans->loai = 'Đến';
            $vanbans->noidung_butphe = $conten_butphe;

            // get danh sách emails gửi đến
            $userIdsSendMail = [];
            foreach($userIds_chutri as $userId_chutri) {
                if (!in_array($userId_chutri, $sentUserIds)) {
                    $userIdsSendMail[] = $userId_chutri;
                }
            }
            $vanbans->noinhan = User::select('email')->whereIn('id', $userIdsSendMail)->pluck('email')->toArray();

            // send mail
            $this->dispatch(new SendMail($vanbans->toArray()));
        }
    ...
    public function quy_trinh_chuyen_tiep()
    {
        ...
        return view(config('app.interface').'vanban.quy_trinh_chuyen_tiep', compact('vanBanXuLy', 'vb_child'));
    }
    ...
    public function xu_ly_van_ban_user()
    {
        ...
        return view(config('app.interface').'vanban.xu_ly_van_ban_user', compact('id_vanbanuser', 'id'));
    }
    ...
    public function check_xu_ly_vanban() {
        ...
        if (!$vanbanxuly) {
            return view(config('app.interface').'vanban._form_check_status_vanban', compact('vanbanxuly'));
        }
        ...
        return view(config('app.interface').'vanban._form_check_status_vanban', compact('vanbanxuly', 'check'));
    }
    ...
    public function nhap_vanban_banhanh() {
        ...
        return view(config('app.interface').'vanban.form_van_ban_banhanh', compact('donvis'));
    }
    ...
    public function sua_vanban_banhanh($vanbanbanhanhId) {
        ...
        return view(config('app.interface').'vanban.form_van_ban_banhanh', compact('donvis', 'vanbanBanhanh', 'userNhanIdsArr', 'usersInDonvi'));
    }
    ...
    public function danhsach_vanban_banhanh() {
        ...
        return view(config('app.interface').'vanban.danh_sach_van_ban_ban_hanh', compact('vanbans', 'users','donvis', 'type'));
    }
    public function thuhoivanban() {
        $id = Input::get('id');
...
//        $this->dispatch(new SendMailGanHetHanXuLy($vanban->toArray()));
        ...
        $vanbans = VanbanXuLy::where('vanban_xulys.status', '<>', 3)
            ...
            ->Where('users.donvi_id' != 151)
            ->where('vanban_xulys.reminder', 0)
            ->where('vanban_xulys.type', 1)
            ->whereBetween(\DB::raw("STR_TO_DATE(vanbans.hanxuly,'%Y-%m-%d')"),array($curDate, date('Y-m-d', strtotime( date('Y-m-d'). ' + 3 days'))))
            ->get();
        if (sizeof($vanbans) > 0){
            foreach ($vanbans as $vanban) {
                $vanban['link'] = url('vanban/chi-tiet-van-ban-den/' . $vanban['idvanbanden']);
                $vanban['path'] = url('vanban/dowload_file/');
