...
class BaocaoController extends Controller
{
    ...
    function guibaocao(){
        ...
        return view(config('app.interface').'baocaos.guibaocao',compact('checkreport','day','kehoachs'));
    }
...
//        $data = $request->all();
        ...
        return view(config('app.interface').'baocaos.xem_truoc_noi_dung', compact('content'));
    }
    public function xem_noi_dung_bao_cao(){
       ...
       // dd($content);die();
        return view(config('app.interface').'baocaos.xem_noi_dung', compact('content','date'));
    }
    public function bao_cao_da_gui(){
        ...
        return view(config('app.interface').'baocaos.bao_cao_da_gui',compact('reports','start_date'));
    }
    ...
    public function sua_bao_cao($id){
        ...
        $kehoachs = Plan::select('plans.*')
            ...
            ->where('plans.end_day','>=',$day)
            ->first();
        return view(config('app.interface').'baocaos.sua_bao_cao',compact('report','kehoachs'));
    }
    // Cập nhật báo cáo
    ...
    public function congviecOld(Request $request){
        ...
        if ($request->ajax == 1) {
            return view(config('app.interface').'congviecs._table_congviec',compact('congviecs'));
        }
        ...
        return view(config('app.interface').'baocaos.xem_bao_cao_don_vi',compact('reports','user','start_date','kehoachs','congviecs','limit','trangthai_search','donvis','dv_sess'));
    }
    ...
    public function congviec(Request $request) {
        ...
        return view(config('app.interface').'baocaos.xem_bao_cao_don_vi', compact('congviecs', 'donvis'));
    }
    ...
    public function kq_congviec(){
        ...
        return view(config('app.interface').'baocaos.xem_bao_cao_don_vi',compact('reports','user','start_date','kehoachs'));
    }
    public function len_ke_hoach(){
        ...
        return view(config('app.interface').'baocaos.len_ke_hoach',compact('start_date','end_date','kehoachs'));
    }
    ...
    public function xem_ke_hoach(){
        ...
        return view(config('app.interface').'baocaos.xem_ke_hoach', compact('content','start','end'));
    }

    public function sua_ke_hoach($id){
        $plans = Plan::find($id);

       return view(config('app.interface').'baocaos.sua_ke_hoach',compact('plans'));
    }
    ...
    public function xem_chi_tiet_ke_hoach(){
        ...
        return view(config('app.interface').'baocaos.xem_chi_tiet_ke_hoach', compact('plans'));
    }
    ...
    public function viewDetail_report(Request $request){
        ...
        return view(config('app.interface').'baocaos._modalViewDetail_Report',compact('congviec_baocao','congviec_user'));
    }
    ...
    public function commonBaocao_congviecs(Request $request) {
        ...
        if ($congviec->user_id == session('user')['id']) {// nếu là người giao công việc: xem danh sách báo cáo
            ...
            return view(config('app.interface').'baocaos._showOrduyetBaocao_congviec_dagiao',compact('congviec_users','congviec_id'));
        }
            ...
            return view(config('app.interface').'baocaos._showBaocao_congviec_duocgiao',compact('congviec_users','congviec_id'));
        }
    }
    ...
    public function view_baocao_notifi(Request $request){
        ...
        $congviec_user->save();
        return view(config('app.interface').'baocaos.modal_baocao_notifi',compact('congviec_user'));
    }
    ...
    public function view_agreement_report(Request $request){
        ...
        $congviec_baocao->save();
        return view(config('app.interface').'baocaos._modal_agreement_report',compact('congviec_baocao'));
    }
    public function dashboard (){
        $vanbans = 0;
      return view(config('app.interface').'baocaos.dashboard',compact('vanbans'));
    }
    ...
    public function tinh_hinh_xu_ly_van_ban_den() {
        ...
        return view(config('app.interface').'baocaos.tinh_hinh_xu_ly_van_ban_den', compact('ds_vanbans', 'total'));
    }
    ...
    public function tong_hop_tinh_hinh_xu_ly_van_ban() {
        ...
        return view(config('app.interface').'baocaos.tong_hop_tinh_hinh_xu_ly_van_ban',compact('data'));
    }
    ...
    public function baoCaoThongKeDonVi(Request $request)
    {
        ...
        if ($request->start_time){
            $start_time = formatYMD($request->start_time) . ' 00:00:00';
        }
        if ($request->end_time){
            $end_time = formatYMD($request->end_time) . ' 23:59:59';
        }
        $filter_sql = '';
        ...
        $data = VanbanXuLy::select(
                ...
                'tbDonViNhan.name as tenDonViNhan',
                DB::raw('
                        COUNT(CASE WHEN vanban_xulys.STATUS = 3
                        AND vanban_xulys.donvi_nhan_id = tbDonViNhan.id
                        AND ((vanban_xulys.ngayxuly
                        AND vanbans.hanxuly
                        AND DATE(vanban_xulys.ngayxuly) <= DATE(vanbans.hanxuly))
                        OR ! vanbans.hanxuly) THEN 1 END) tongVBHoanThanhDungHan
                    '),
                DB::raw('
                        COUNT(CASE WHEN vanban_xulys.STATUS = 3
                        AND vanban_xulys.donvi_nhan_id = tbDonViNhan.id
                        AND vanbans.hanxuly
                        AND DATE(vanban_xulys.ngayxuly) > DATE(vanbans.hanxuly) THEN 1 END) tongVBHoanThanhQuaHan
                    '),
                DB::raw('
                        COUNT(CASE WHEN vanban_xulys.STATUS <> 3
                        AND vanban_xulys.donvi_nhan_id = tbDonViNhan.id
                        AND vanbans.hanxuly
                        AND CURDATE() > DATE(vanbans.hanxuly) THEN 1 END) tongVBChuaHoanThanh
                    '),
                DB::raw('COUNT(CASE WHEN vanban_xulys.donvi_nhan_id = tbDonViNhan.id AND vanban_xulys.type = 1 THEN 1 END) tongVBChuTri'),
                DB::raw('COUNT(CASE WHEN vanban_xulys.donvi_nhan_id = tbDonViNhan.id AND vanban_xulys.type = 2 THEN 1 END) tongVBPhoiHop')
            )
        ...
        if ($exportWord){
            $type = 1;
            $donviName = '';
            return view(config('app.interface').'baocaos.bao_cao_thong_ke_theo_vb_den_word', compact('data', 'start_time', 'end_time', 'type', 'donviName'));
        }else{
            return view(config('app.interface').'baocaos.bao_cao_thong_ke_theo_don_vi', compact('data', 'start_time', 'end_time'));
        }
    }
...
//                    WHERE `id_nhan` = users.id';
        ...
        $data = VanbanXuLy::select(
            ...
            DB::raw('
                    ...
                    AND CURDATE() > DATE(vanbans.hanxuly) THEN 1 END) tongVBChuaHoanThanh
                '),
            DB::raw('COUNT(CASE WHEN vanban_xulys.type = 1 THEN 1 END) tongVBChuTri'),
            DB::raw('COUNT(CASE WHEN  vanban_xulys.type = 2 THEN 1 END) tongVBPhoiHop')
        )
            ->join('vanbans', function ($join) {
                $join->on('vanbans.id', '=', 'vanban_xulys.vanbanUser_id')
                    ->where('vanbans.book_id', 1);
        ...
        $donvis = Donvi::where('donvis.actived',1)
            ->where('donvis.id', '!=', 151)
            ->join('book_detail', function($join) {
        ...
        if ($exportWord){
            ...
            $type = 2;
            return view(config('app.interface').'baocaos.bao_cao_thong_ke_theo_vb_den_word', compact('data', 'donviName', 'start_time', 'end_time', 'type'));
        }else{
            return view(config('app.interface').'baocaos.bao_cao_thong_ke_theo_ca_nhan', compact('data', 'donvis', 'donvi_id', 'start_time', 'end_time'));
        }
    }
    ...
    public function xemBaoCaoThongKe(Request $request)
    {
        ...
        $start_time = formatYMD($input['start_time']) . ' 00:00:00';
        $end_time = formatYMD($input['end_time']) . ' 23:59:59';
        ...
        if($input['type'] == 5){
            $query->whereRaw("
                vanban_xulys.status <> 3
                AND vanban_xulys.type = 1
                AND ((vanbans.hanxuly AND (CURDATE() > DATE(vanbans.hanxuly) OR vanban_xulys.ngayxuly IS NULL)) OR (vanban_xulys.ngayxuly IS NULL AND vanbans.hanxuly = '' OR vanbans.hanxuly IS NULL) )
            ");
        }
        ...
        $data = $query->join('vanbans', function ($join) {
            ...
            ->orderBy('ngaychuyentiep', 'DESC')
            ->get(['vanbans.id', 'vanbans.title', DB::raw('DATE_FORMAT(vanban_xulys.ngaychuyentiep,"%d-%m-%Y %H:%i:%s") AS ngaychuyentiep'),  DB::raw('DATE_FORMAT(vanban_xulys.ngayxuly,"%d-%m-%Y %H:%i:%s") AS ngayxuly'), 'vanban_xulys.status', 'vanban_xulys.type']);
    ...
    public function thong_ke_van_ban_den_theo_ngay() {
        ...
        return view(config('app.interface').'baocaos.thong_ke_van_ban_den_theo_ngay', compact('curDate', 'ds_vanbans'));
    }

    public function baoCaoThongKeDonVi_vbnoibo(Request $request){
        if(session('user')['view_thongke'] !== 1) return abort(403);
        $exportWord = Input::get('export_word');
        $start_time = date('Y-m-01 00:00:00');
        $end_time = date('Y-m-d 23:59:59');
        ...
        $_sql_cm = 'SELECT count(*)
                    FROM `vanban_xulys`
                    JOIN `vanbans` ON `vanbans`.`id` = `vanban_xulys`.`vanbanUser_id` AND `vanbans`.`book_id` = 4
                    WHERE `is_chuyentiep` = 0
                    AND `donvi_nhan_id` = tbDonViNhan.id';
        ...
        if ($exportWord){
            $type = 1;
            $donviName = '';
            return view(config('app.interface').'baocaos.bao_cao_thong_ke_theo_vb_noi_bo_word', compact('data', 'start_time', 'end_time', 'type', 'donviName'));
        }else{
            return view(config('app.interface').'baocaos.thong_ke_van_ban_noi_bo_theo_don_vi', compact('data', 'start_time', 'end_time'));
        }
    }

    public function baoCaoThongKeCaNhan_vbnoibo(Request $request){
        if(session('user')['view_thongke'] !== 1) return abort(403);
        $exportWord = Input::get('export_word');
        $start_time = date('Y-m-01 00:00:00');
        $end_time = date('Y-m-d 23:59:59');
        ...
        if ($exportWord){
            ...
            $type = 2;
            return view(config('app.interface').'baocaos.bao_cao_thong_ke_theo_vb_noi_bo_word', compact('data', 'donviName', 'start_time', 'end_time', 'type'));
        }else{
            return view(config('app.interface').'baocaos.thong_ke_van_ban_noi_bo_theo_ca_nhan', compact('data','donvis', 'donvi_id', 'start_time', 'end_time'));
        }
    }

    public function xemBaoCaoThongKe_vbnoibo(Request $request){
        $input = $request->only('donvi_id', 'user_id', 'type', 'start_time', 'end_time');
        $start_time = formatYMD($input['start_time']) .' 00:00:00';
        $end_time = formatYMD($input['end_time']) .' 23:59:59';
        ...
        $data = $query->join('vanbans', function ($join) {
            ...
            ->orderBy('ngaychuyentiep', 'DESC')
            ->get(['vanbans.id', 'vanbans.title', 'vanbans.file_dinhkem', DB::raw('DATE_FORMAT(vanban_xulys.ngaychuyentiep,"%d-%m-%Y %H:%i:%s") AS ngaychuyentiep'),  DB::raw('DATE_FORMAT(vanban_xulys.ngayxuly,"%d-%m-%Y %H:%i:%s") AS ngayxuly'), 'vanban_xulys.status', 'vanban_xulys.type']);
        return  response()->json($data);
    }

    function thongKeVBDen(Request $request){
        $exportWord = Input::get('export_word');
        $start_time = date('Y-m-01 00:00:00');
        $end_time = date('Y-m-d 23:59:59');
        if ($request->start_time){
        ...
        $data = VanbanXuLy::select(
            ...
            DB::raw('
                        ...
                        AND vanban_xulys.donvi_nhan_id = tbDonViNhan.id
                        AND vanbans.hanxuly
                        AND ( CURDATE() > DATE(vanbans.hanxuly) OR vanban_xulys.ngayxuly is NULL) THEN 1 END) tongVBChuaHoanThanh
                    '),
            DB::raw('
                        COUNT(CASE WHEN vanban_xulys.STATUS <> 3
                        AND vanban_xulys.type = 1
                        AND vanban_xulys.ngayxuly IS NULL 
                        AND vanbans.hanxuly = "" 
                        OR vanbans.hanxuly IS NULL 
                        AND vanban_xulys.donvi_nhan_id = tbDonViNhan.id THEN 1 END) tongVBChuaHT
                    '),
        ...
        ->where('tbDonViNhan.id', '!=', 151)
        ->where('tbDonViNhan.parent_id', '=', 1)
        ->orderBy('tbDonViNhan.order', 'ASC')
        ->groupBy('tbDonViNhan.id')
        ->orderBy('tenDonViNhan', 'ASC')
        ->paginate(30)
        ->appends(Input::except('page'));
        if ($exportWord){
            $type = 1;
            $donviName = '';
            return view(config('app.interface').'thongke.van_ban_den_word', compact('data', 'start_time', 'end_time', 'type', 'donviName'));
        }else{
            return view(config('app.interface').'thongke.van_ban_den', compact('data', 'start_time', 'end_time'));
        }
    }

    function thongKeVBDenCaNhan(Request $request){
        $exportWord = Input::get('export_word');
        $start_time = date('Y-m-01 00:00:00');
        $end_time = date('Y-m-d 23:59:59');
        if ($request->start_time){
        ...
        if ($exportWord){
            ...
            $type = 2;
            return view(config('app.interface').'thongke.van_ban_den_word', compact('data', 'donviName', 'start_time', 'end_time', 'type'));
        }else{
            return view(config('app.interface').'thongke.van_ban_den_ca_nhan', compact('data', 'donviName', 'donvi_id', 'start_time', 'end_time', 'type'));
        }
    }

    function thongKeVBNoiBo(Request $request){
        if(session('user')['view_thongke'] !== 1) return abort(403);
        $exportWord = Input::get('export_word');
        $start_time = date('Y-m-01 00:00:00');
        $end_time = date('Y-m-d 23:59:59');
        ...
        if ($exportWord){
            $type = 1;
            $donviName = '';
            return view(config('app.interface').'thongke.van_ban_noi_bo_word', compact('data', 'start_time', 'end_time', 'type', 'donviName'));
        }else{
            return view(config('app.interface').'thongke.van_ban_noi_bo', compact('data', 'start_time', 'end_time'));
        }
    }

    function thongKeVBNoiBoCaNhan(Request $request){
        if(session('user')['view_thongke'] !== 1) return abort(403);
        $exportWord = Input::get('export_word');
        $start_time = date('Y-m-01 00:00:00');
        $end_time = date('Y-m-d 23:59:59');
        ...
        if ($exportWord){
            ...
            $type = 2;
            return view(config('app.interface').'thongke.van_ban_noi_bo_word', compact('data', 'donviName', 'start_time', 'end_time', 'type'));
        }else{
            return view(config('app.interface').'thongke.van_ban_noi_bo_ca_nhan', compact('data', 'donviName', 'donvi_id', 'start_time', 'end_time', 'type'));
        }
    }

    function thongKeVBDi(Request $request){
        $exportWord = Input::get('export_word');
        ...
        $data = Vanban::select(
            'donvis.id AS donvi_id',
            'donvis.name as tenDonviBH',
            DB::raw('COUNT(CASE WHEN vanbans.cq_banhanh = donvis.id THEN 1 END) tongVBBanHanh')
        )
            ->where('vanbans.book_id', 2)
            ->where('vanbans.cq_banhanh', '!=', 151)
            ->join('donvis', 'donvis.id', '=', 'vanbans.cq_banhanh')
            ->when($start_time, function ($query) use ($start_time) {
                return $query->where('vanbans.ngayky', '>=', $start_time);
            })
            ->when($end_time, function ($query) use ($end_time) {
                return $query->where('vanbans.ngayky', '<=', $end_time);
            })
            ->where('donvis.id', '!=', 151)
            ->where('donvis.parent_id', '=', 1)
            ->orderBy('donvis.order', 'ASC')
            ->groupBy('donvis.id')
            ->orderBy('tenDonviBH', 'ASC')
            ->paginate(30)
            ->appends(Input::except('page'));
        ...
        if ($exportWord){
            $donviName = '';
            $type = 1;
            return view(config('app.interface').'thongke.van_ban_di_word', compact('data', 'donviName', 'start_time', 'end_time', 'type'));
        }else{
            return view(config('app.interface').'thongke.van_ban_di', compact('data', 'start_time', 'end_time'));
        }
    }

    function thongKeVBDiList(Request $request){
//        if(session('user')['view_thongke'] !== 1) return abort(403);
        $exportWord = Input::get('export_word');
        $donvi_id = $request->donvi_id;
        $donvis = Donvi::where('donvis.actived',1)
        ...
        if ($exportWord){
            ...
            return view(config('app.interface').'thongke.van_ban_di_list_nhan_vien', compact('data', 'donvi_id', 'donviName'));
        }
    }

    public function xemBaoCaoThongKeVBDi(Request $request){
        $input = $request->only('donvi_id', 'start_time', 'end_time');
        $start_time = formatYMD($input['start_time']);
        $end_time = formatYMD($input['end_time']);

        $data = Vanban::select('id', 'title', 'ngayky')
        ->where('cq_banhanh', $input['donvi_id'])
        ->where('ngayky', '>=', $start_time)
        ->where('ngayky', '<=', $end_time)
        ->where('book_id', 2)
        ->orderBy('ngayky', 'DESC')
        ->get();
        return  response()->json($data);
