...
function formatM_Y($dateTime)
{
    return ($dateTime) ? date('m_Y', strtotime(str_replace('-', '/', $dateTime))) : '';
}
...
function sendMailMailer($data, $template, $subject, $toUsers, $fromUser = null)
{
    ...
    // create content
    $content = (string)\Illuminate\Support\Facades\View::make($template, $data)->render();
    ...
    try {
        ...
        $mail->SMTPAuth = true;
        $mail->Username =  $fromUser->email;
        $mail->Password = 'Udn@41leduan';
        $mail->SMTPSecure = config('mail.encryption');
        $mail->Port = config('mail.port');
        $mail->setFrom($fromUser->email, 'Điều Hành Tác Nghiệp');
        //Recipients
        if ($toUsers != 'nhacnho'){
            ...
            foreach ($data as $val){
                ...
                foreach ($files as $file){
                    $mail->addAttachment('./files/vanban/' . $file);
                }
                $mail->addAddress($val['email'], isset($val['tenUserNhan']) ? $val['tenUserNhan'] : '');
            }
        }
        //Content
        $mail->isHTML(true);
...
//            $mail->addAddress(config('mailluong.from.address'), isset($data->fullname) ? $data->fullname : '');
            ...
            $mail->SMTPSecure = 'tls';
            $mail->SMTPAuth   = true;
            $mail->Username = 'bankhtc2@ac.udn.vn';
            $mail->Password = 'Tri26noi06@';
            $mail->SetFrom('bankhtc2@ac.udn.vn', 'Điều Hành Tác Nghiệp');
