...
class VanbanXuLy extends Model
{
    ...
    public function scopeGetDanhSachNoiBoGuiForSearch($query, $donviId, array $options = []) {
        $query->selectColumns()->with([
                    'vanban' => function ($q) {
                        $q->selectColumnsInWith();
                    },
        ...
        if  (isset($options['status']) && $options['status']) {
            $query->where('vanban_xulys.status', 'like' , $options['status']);
        }
		
        return $query;
    }

    public function scopeGetDanhSachNoiBoNhanForSearch($query, $userId , array $options = []) {
        $query->selectColumns()->with([
                    'vanban' => function ($q) {
                        $q->selectColumnsInWith();
                    },
    ...
    public function scopeGetVBdenVBNoiBoVBForSeach($query, $userId , $donviId, array $options = []) {
        $query->selectColumns()->with([
            'vanban' => function ($q) {
                $q->selectColumnsInWith();
            },
        ...
        ->join('vanbans', 'vanbans.id', '=', 'vanban_xulys.vanbanUser_id')
                ->whereIn('vanbans.book_id', [1, 4])
                ->join('users', 'users.id', '=', 'vanbans.user_id')
                ->where(function ($q) use ($userId, $donviId) {
                    $q->where('vanban_xulys.id_nhan', $userId)
                      ->orWhere('users.donvi_id', $donviId);
                })
                ->where('vanban_xulys.parent_id', '>=', 0)
                ->orderBy('vanbans.ngayden', 'DESC');
    ...
    public function scopeGetDanhSachDenForSearch($query, $userId, array $options = []) {
        $query->selectColumns()->with([
                    'vanban' => function ($q) {
                        $q->selectColumnsInWith();
                    },
