...
class Lichtuan extends Model
{
    ...
    public function scopeGetDanhSach($query, array $options = []) {
        ...
        ])->select('lichtuans.*','users.fullname as tenchutri','users.chucdanh')->leftJoin('users', 'users.id', '=', 'lichtuans.chutriId')->orderBy('lichtuans.time', 'ASC');
        ...
        if(isset($options['exportWord']) && $options['exportWord'] == 1 ) {
            $query->where('lichtuans.status', 1);
        }else {
        ...
        if (isset($options['time']) && $options['time']) {
            $query->where('lichtuans.time', '=', $options['time'].' 00:00:00');
        }
