    ...
    <div class="container container-list">
        ...
        <div class="row">
                ...
                @if ($type === 'nhan')
                    <ul class="nav nav-pills">
                        <li class="bg_luanchuyen {{ $status == 'chuaxuly'? 'active' : '' }}">
                            <a href="{{ url('vanbannoibo/danh-sach-nhan').'?status=chuaxuly&?type=nhan' }}">Chưa xử lý</a>
                        </li>
                        <li class="bg_luanchuyen {{ $status == 'dangxuly'? 'active' : '' }}">
                            <a href="{{ url('vanbannoibo/danh-sach-nhan').'?status=dangxuly&?type=nhan' }}">Đang xử lý</a>
                        </li>
                        <li class="bg_luanchuyen {{ $status == 'daxuly'? 'active' : '' }}">
                            <a href="{{ url('vanbannoibo/danh-sach-nhan').'?status=daxuly&?type=nhan' }}">Đã xử lý</a>
                        </li>
                        <li class="bg_luanchuyen {{ $status == 'ganhethan'? 'active' : '' }}">
                            <a href="{{ url('vanbannoibo/danh-sach-nhan').'?status=ganhethan&?type=nhan' }}">Báo hết hạn</a>
                        </li>
                        <li class="bg_luanchuyen {{ (!$status || $status == 'all')? 'active' : '' }}">
                            <a href="{{ url('vanbannoibo/danh-sach-nhan').'?status=all&?type=nhan' }}">Tất cả</a>
                        </li>
                    </ul>
                @endif
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                @if ($val->vanban->user_id == session('user')['user_id_login'])
                                    <span style="padding:5px 10px;cursor: pointer;"><a onclick="thuHoiVanBan({{$val->vanbanUser_id}})" title="Thu hồi văn bản"><i class="text-danger fa fa-window-close-o"> </i></a></span>
                                @endif
                            </td>
                        </tr>
                    @endforeach
            ...
            <div class="pull-right">
                @include(config('app.interface').'pagination', ['paginator' => $vanbans, 'interval' => 5])
            </div>
        </div>
    </div>
        ...
        function thuHoiVanBan(id) {
            jConfirm('Bạn có muốn thu hồi văn bản này?', 'Thông báo', function (r) {
                if (r) {
                    ...
                    let CSRF_TOKEN = $('meta[name="csrf-token"]').attr('content');
                    $.post("{{ route('vanban.thuhoivanban') }}", {id:[id], _token:CSRF_TOKEN}, () => {
                        location.reload();
                    }, 'json');
                }
            });
