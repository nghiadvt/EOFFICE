<div class="col-md-7 wrapper-left-content">
	...
	<div style="float: left;margin-top: 20px;">
		@include(config('app.interface').'partials._pagination_limit_selector')
	</div>
	...
	<div class="table-responsive" id="congviec_table">
		@if( sizeof($congviecs) > 0 )
			@include(config('app.interface').'congviecs._table_congviec')
		@else
			<em style="display: table; margin: 30px auto;">Không có công việc</em>
		@endif
		@include(config('app.interface').'partials._pagination',['list'=>$congviecs, 'ajax'=>1, 'container'=>'#congviec_table'])
	</div>
</div>
<?php 
	...
	<div class="tab-content" id="list-content">
		<div id="detail-tab" class="tab-pane fade in active">
			<div class="container-work-detali">
				@include(config('app.interface').'tabs.chitiet')
			</div>
		</div>
		<div id="document-tab" class="tab-pane fade">
			@include(config('app.interface').'tabs.tailieu')
		</div>
		<div id="discuss-tab" class="tab-pane fade">
			@include(config('app.interface').'tabs.thaoluan')
		</div>
		<div id="report-tab" class="tab-pane fade">
			@include(config('app.interface').'tabs.baocao')
		</div>
		<div id="participant" class="tab-pane fade">
			@include(config('app.interface').'tabs.nguoithamgia')
		</div>
		
	</div>
</div>
@include(config('app.interface').'congviecs.them-cv')
@include(config('app.interface').'congviecs.capnhat_cv')
<script>
