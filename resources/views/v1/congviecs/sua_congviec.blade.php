		...
		<form action="" id="form_edit_work" method="POST">
		    ...
		    <div class="modal-body">
		    	<div class="form-group">
		    		...
		    		<input id="work-editing" type="text" name="tencongviec" class="form-control" value="{{ $congviec->tencongviec }}">
		    	</div>
		    	<div class="form-group">
		    	...
		    	<div class="form-group">
			    	<div class="col-xs-6 pd_none_left">
					    <div class="form-group">
					    	...
					    	<input id="begin-work-editing"  type="text" class="form-control datepicker" value="{{ formatDMY($congviec->ngaybatdau) }}" name="ngaybatdau">
					    </div>	
			    	</div>
			    	<div class="col-xs-6 pd_none_right">
					    <div class="form-group">
					    	<label>Ngày kết thúc:</label>
					    	<input id="end-work-editing"  type="text" class="form-control datepicker" value="{{ formatDMY($congviec->ngayketthuc) }}" name="ngayketthuc">
					    </div>	
			    	</div>
			    </div>
		        <div class="form-group">
                ...
                <div class="form-group">
			    	...
			    	<select id="status-work-editing"  name="trangthai" class="form-control">
        ...
        $('form#form_edit_work').ajaxForm({
        	...
        	success: function(data){
        		if (data == 'done') {
					...
					var works_editing = document.querySelectorAll('.work-editing');
				    works_editing.forEach((work) => {
						if (work.getAttribute('data-congviecIds') == '{{ $congviec->id }}' ) {
							var newWorkEditting = document.querySelector('#work-editing')
							var newBeginWorkEditting = document.querySelector('#begin-work-editing')
							var newEndWorkEditting = document.querySelector('#end-work-editing')
							var newStatusWorkEditting = document.querySelector('#status-work-editing')
							work.children[1].innerText = newBeginWorkEditting.value
							work.children[2].innerText = newEndWorkEditting.value
							work.children[3].innerText = newWorkEditting.value
							work.children[4].innerText = newStatusWorkEditting.value
						}
					})
        			// reloadAjax();
        		}else{
