            ...
            <div role="form">
                <div class="tab-content">
                            ...
                            <form id="congviec_phancong" action="{{ url('congviec/phancong') }}" method="POST"
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea type="text" class="form-control"
                                              name="nd_chitiet" id="nd_chitiet">{{ old('nd_chitiet') }}</textarea>
                                </div>
                                <div class="form-group">
                    ...
                    <div class="tab-pane" role="tabpanel" id="step3">
                        @include(config('app.interface').'congviecs.congviecfile')
                    </div>
...
<div id="modal_choose_giahan" class="modal fade" tabindex="-1" role="dialog">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                <h4 class="modal-title">Gia hạn công việc</h4>
            </div>
            <div class="modal-body">
                <form id="congviec_giahan" method="POST" action="{{url('congviec_chitiet/giahancongviec')}}">
                    {{ csrf_field() }}
                    <input type="hidden" id="idcongviec" name="idcongviec">
                    <div class="form-group">
                        <label for="">Ngày kết thúc:</label>
                        <input type="date" id="ngayketthuccongviec" disabled value="" class="form-control">
                    </div>
                    <div class="form-group">
                        <label>Thời gian gia hạn</label>
                        <input type="text" class="form-control datepicker" name="ngaygiahan" id="ngaygiahan" value="">
                    </div>
                    <div class="form-group">
...
<script>
    ...
    $(document).ready(function () {
        ...
        $("#create_work").validate({
            ...
            messages: {
                ...
                ngaybd: {
                    ...
                    currentDate: "Ngày bắt đầu không được nhỏ hơn ngày hiện tại"
                },
                ngaykt: {
                    required: "Vui lòng nhập ngày kết thúc",
                    customDateValidation: "Ngày kết thúc không được nhỏ hơn ngày bắt đầu"
                },
        ...
        $.validator.addMethod("customDateValidation", function(value, element) {
            var startDate = formattedDate($("#ngaybd").val());
            var endDate = formattedDate(value);
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $.each(data.congviecCT, function (index, item) {
                    ...
                    tbody += '<td><i onclick="gia_han_cong_viec('+item.id+ ',' +'`'+ item.ngayketthuc + '`' +')" style="cursor: pointer" title="Gia hạn công việc" class="fa fa-calendar"></i></td>';
                    tbody += '</tr>';
                });
    ...
    function gia_han_cong_viec(id, ngayketthuc){
        $('#ngayketthuccongviec').val(ngayketthuc)
        $('#idcongviec').val(id)
        $('#modal_choose_giahan').modal('show')
    }

    $('#congviec_giahan').validate({
        rules: {
            ngaygiahan: {
                required: true
            },
