...
@section('main')
    ...
    <div class="container container-list report-statistic">
        ...
        <div class="row">
            <div class="box-head">
                <h3>Báo cáo thống kê văn bản nội bộ - <small>Thời điểm xuất báo cáo: {{date('d/m/Y')}}</small></h3>
                <div>
                    ...
                    @if(count($data) > 0)
                        @foreach ($data as $val)
                            <tr data-donvi-id="{{ $val->donvi_id }}">
                                ...
                                <td class="cursor-pointer detail-donvi"><a href="{{route('thongke.vanbannoibocanhan', ['donvi_id' => $val->donvi_id, 'start_time' => $start_time, 'end_time' => $end_time])}}">{{ $val->tenDonViNhan }}</a></td>
    ...
    <script type="text/javascript">
        ...
        $('.ds-bao-cao').click(function(event) {
            ...
            $.getJSON('{{ route("baocaothongke.vbnoibodanhsach") }}', {donvi_id: donvi_id, type: type, start_time: start_time, end_time: end_time}, function(json, textStatus) {
                loading_hide();
                ...
                if(json.length == 0) {
                    ...
                    <td><a href="{{url('vanbannoibo/chi-tiet')}}/${val.id}" target="_blank">${val.title}</a></td>
        ...
        function xuatWord() {
            let url = "<?php echo e(route('thongke.vanbannoibo')); ?>";
