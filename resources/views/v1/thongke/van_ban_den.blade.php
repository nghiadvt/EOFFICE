...
@section('main')
                    ...
                    @if(count($data) > 0)
                        @foreach ($data as $val)
                            <tr data-donvi-id="{{ $val->donvi_id }}">
                                ...
                                <td class="cursor-pointer detail-donvi"><a href="{{ $user->donvi_id == 136 || $user->donvi_id == $val->donvi_id ? route('thongke.vanbandencanhan', ['donvi_id' => $val->donvi_id, 'start_time' => $start_time, 'end_time' => $end_time]) : ''}}">{{ $val->tenDonViNhan }}</a></td>
                                <td data-toggle="tooltip" title="Bấm vào để xem danh sách văn bản" class="cursor-pointer {{$user->donvi_id == 136 || $user->donvi_id == $val->donvi_id ? 'ds-bao-cao' : ''}}" data-type="1">{{ $val->tongVBChuTri }}</td>
                                <td data-toggle="tooltip" title="Bấm vào để xem danh sách văn bản" class="cursor-pointer {{$user->donvi_id == 136 || $user->donvi_id == $val->donvi_id ? 'ds-bao-cao' : ''}}" data-type="2">{{ $val->tongVBPhoiHop }}</td>
                                <td data-toggle="tooltip" title="Bấm vào để xem danh sách văn bản" class="cursor-pointer bg-success {{$user->donvi_id == 136 || $user->donvi_id == $val->donvi_id ? 'ds-bao-cao' : ''}}" data-type="3">{{ $val->tongVBHoanThanhDungHan }}</td>
                                ...
                                <td data-toggle="tooltip" title="Bấm vào để xem danh sách văn bản" class="cursor-pointer bg-warning {{$user->donvi_id == 136 || $user->donvi_id == $val->donvi_id ? 'ds-bao-cao' : ''}}" data-type="4">{{ $val->tongVBHoanThanhQuaHan }}</td>
                                ...
                                <td data-toggle="tooltip" title="Bấm vào để xem danh sách văn bản" class="cursor-pointer bg-danger {{$user->donvi_id == 136 || $user->donvi_id == $val->donvi_id ? 'ds-bao-cao' : ''}}" data-type="5">{{ $val->tongVBChuaHoanThanh + $val->tongVBChuaHT}}</td>
    ...
    <script type="text/javascript">
        ...
        function xuatWord() {
            let url = "<?php echo e(route('thongke.vanbanden')); ?>";
