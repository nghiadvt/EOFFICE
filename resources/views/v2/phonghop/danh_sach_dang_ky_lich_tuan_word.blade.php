...
{{--                            <h4 style="color:red">LỊCH SINH HOẠT CÔNG TÁC TUẦN {{ $weekInYear+ ($dataWeekOfYear->difference +1) }}</h4>--}}
                            <h4 style="color:red">LỊCH SINH HOẠT CÔNG TÁC TUẦN {{ $weekInYear > '30' ? ($weekInYear - 30) : ($dataWeekOfYear->difference + $weekInYear) }}</h4>
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td style="text-align: center;">
                            <h4 style="margin-top: 5px; margin-bottom: 8px;">{{ $firstDateInWeek->format('d-m-Y') }} đến {{$lastDateInWeek->format('d-m-Y') }}</h4>
                            @if($weekInYear > '30')
                                <h4 style="margin: 0;">Năm học {{ $firstDateInWeek->format('Y')  }}-{{ $firstDateInWeek->format('Y') + 1 }}</h4>
                            @else
                                <h4 style="margin: 0;">Năm học {{ $firstDateInWeek->format('Y') - 1  }}-{{ $firstDateInWeek->format('Y') }}</h4>
                            @endif
                        </td>
                    </tr>
                    <tr>
            ...
            <div>
                @if ($type === '' || $type === '1')
                    ...
                    @if(isset($dataDHDN) && sizeof($dataDHDN)>0)
                    <table class="border">
                        <thead>
                            <tr>
                                <th>THỨ/NGÀY</th>
                        ...
                        <tbody style="border-bottom: 1pt solid windowtext;">
                            ...
                            @foreach ($dataDHDN as $ngay => $data)
                                ...
                                <tr class="{{ sizeof($data) > 1 ? 'first-hand' : 'four-hand' }}">
                                    ...
                                    <td style="text-align: center; font-weight: bold">{{ $val->chutriId != '' ? ($val->chucdanh == 'Giám đốc' ? 'Giám đốc ĐHĐN' : 'PGĐ ĐHĐN'. $val->tenchutri )  : $val->chutri}}</td>
                                </tr>
            ...
            <div>
                @if ($type === '' || $type === '2' )
                    ...
                    @if(isset($dataCoquan) && sizeof($dataCoquan)>0)
                        <div>
